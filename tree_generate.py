...
class CustomDirectoryTreeGenerator:
    ...
    def __init__(self, root_dir, exclude_rules=None, ignore_case=True):
        """
        初始化生成器
    ...
    def _generate_tree(self, current_dir, level=0, prefix="", is_last=False):
        ...
        for item in items:
            ...
            # 判断是否需要排除当前项
            if self._is_excluded(item, level + 1):  # 子项层级=当前层级+1
                continue
...
def main():
    ...
    # 3. 生成目录树
    generator = CustomDirectoryTreeGenerator(root_dir, exclude_rules)
